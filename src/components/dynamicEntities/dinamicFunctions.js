...
export function moveRollOverMesh(rollOverMesh, scene, event = null) { //, event = null
  //затем из события применить функцию получения пересечения

  const intersect = getIntersectedObject(event, scene)
  console.log(rollOverMesh, 'rollOverMesh', scene, 'scene');
  // if (!intersect) {
  //   // Если пересечение не передано, устанавливаем объект в начало координат
  //   rollOverMesh.position.set(0, .01, 0);
  // } else {
  //   // Если пересечение передано, обновляем позицию объекта-подсказки в точку пересечения
...
//   //здесь создание, а не перемещение
